...
impl LLMClientTrait for AnthropicClient {
    async fn stream_chat(
        ...
        let url = format!("{}/v1/messages", self.spec.base_url.trim_end_matches('/'));
        let (system, messages) = split_system(&options.messages);
        let mut payload = json!({
            "model": options.model.unwrap_or_else(|| self.spec.id.clone()),
            "max_tokens": options.max_tokens.unwrap_or(self.spec.max_tokens),
            "stream": true,
            "messages": messages,
        });
                    ...
                    for line in frame.lines() {
                        ...
                        match value.get("type").and_then(|v| v.as_str()) {
                            ...
                            Some("content_block_start") => {
                                if value.pointer("/content_block/type").and_then(|v| v.as_str())
                                    == Some("tool_use")
                                {
                                    let id = value
