...
async fn test_heuristic_plugin_selection_triggers() {
    ...
    // 1. Trigger skills
    let res1 = selector.heuristic_select(
        "Can you use a custom skill or playbook for this task?",
        &manifests,
    );
    ...
    // 2. Trigger mcp
    let res2 =
        selector.heuristic_select("Connect to external MCP server to list tools", &manifests);
    assert!(res2.active_plugin_ids.contains(&"mcp".to_string()));

    // 3. Trigger conversation history
    let res3 = selector.heuristic_select(
        "Show previous conversation history from this session",
        &manifests,
    );
